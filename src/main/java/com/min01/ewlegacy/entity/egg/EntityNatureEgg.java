...
public class EntityNatureEgg extends TamableAnimal
{
    public int TimerToHatch = 1000;
    
    public EntityNatureEgg(EntityType<? extends TamableAnimal> type, Level level)
    {
        super(type, level);
    ...
    public void aiStep() 
    {
        ...
        if(this.TimerToHatch == 0 && this.tickCount % 10 == 0)
        {
            for(Entity entity1 : list) 
            {
                ...
                {
                    EntityNatureGolem mob = new EntityNatureGolem(EWEntities.NATURE_GOLEM.get(), this.level);
                	...
                	//TODO
                	//((Player)this.getOwner()).awardStat(MWAWAchievementsRegistry.naturepet2, 1);
                    this.discard();
                }
            }
        }
    ...
    {
        if(p_27584_.getAbilities().instabuild && p_27584_.isHolding(EWItems.NATURE_SHARD.get())) 
        {
            this.TimerToHatch = 3;
        }
        if(this.isTame()) 
