...
public class EntityWaterEgg extends TamableAnimal
{
    public int TimerToHatch = 1000;
    
    public EntityWaterEgg(EntityType<? extends TamableAnimal> type, Level level)
    {
        super(type, level);
    ...
    public void aiStep() 
    {
        ...
        if(this.TimerToHatch <= 0 && this.tickCount % 10 == 0)
        {
            for(Entity entity1 : list) 
            {
                ...
                {
                    EntityAirPhoenix mob = new EntityAirPhoenix(EWEntities.WATER_NYMPH.get(), this.level);
                	...
                	//TODO
                	//((Player)this.getOwner()).awardStat(MWAWAchievementsRegistry.waterpet2, 1);
                    this.discard();
                }
            }
        }
    ...
    {
        if(p_27584_.getAbilities().instabuild && p_27584_.isHolding(EWItems.WATER_SHARD.get())) 
        {
            this.TimerToHatch = 3;
        }
        if(this.isTame()) 
