...
public class EntityIceEgg extends TamableAnimal
{
    public int TimerToHatch = 1000;
    
    public EntityIceEgg(EntityType<? extends TamableAnimal> type, Level level)
    {
        super(type, level);
    ...
    public void aiStep() 
    {
        ...
        if(this.TimerToHatch == 0 && this.tickCount % 10 == 0)
        {
            for(Entity entity1 : list) 
            {
                ...
                {
                    EntityIceBeast mob = new EntityIceBeast(EWEntities.ICE_BEAST.get(), this.level);
                	...
                	//TODO
                	//((Player)this.getOwner()).awardStat(MWAWAchievementsRegistry.icepet2, 1);
                    this.discard();
                }
            }
        }
    ...
    {
        if(p_27584_.getAbilities().instabuild && p_27584_.isHolding(EWItems.ICE_SHARD.get())) 
        {
            this.TimerToHatch = 3;
        }
        if(this.isTame()) 
