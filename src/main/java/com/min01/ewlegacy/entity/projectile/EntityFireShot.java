...
public class EntityFireShot extends AbstractEWHurtingProjectile
{
    public EntityFireShot(EntityType<? extends AbstractEWHurtingProjectile> type, Level level)
    {
        super(type, level);
    }
    
    public EntityFireShot(Level level, LivingEntity living, double x, double y, double z)
    {
        super(EWEntities.FIRE_SHOT.get(), level, living, x,  y, z);
    }
    
    public EntityFireShot(Level level, double x, double y, double z, double motionX, double motionY, double motionZ)
    {
        super(EWEntities.FIRE_SHOT.get(), level, x, y, z, motionX, motionY, motionZ);
    }
    ...
    protected void onHitEntity(EntityHitResult hitResult) 
    {
        if(!this.level.isClientSide)
        {
            if(hitResult.getEntity() != null) 
            {
                ...
                LargeFireball entitywitherskull2 = new LargeFireball(EntityType.FIREBALL, this.level);
                LargeFireball entitywitherskull3 = new LargeFireball(EntityType.FIREBALL, this.level);
                LargeFireball entitywitherskull4 = new LargeFireball(EntityType.FIREBALL, this.level);
                ...
                entitywitherskull2.setDeltaMovement(entitywitherskull2.getDeltaMovement().subtract(0, 5.0, 0));
                entitywitherskull3.setDeltaMovement(entitywitherskull3.getDeltaMovement().subtract(0, 5.0, 0));
                entitywitherskull4.setDeltaMovement(entitywitherskull4.getDeltaMovement().subtract(0, 5.0, 0));
                entitywitherskull5.setDeltaMovement(entitywitherskull5.getDeltaMovement().subtract(0, 5.0, 0));
