...
public class EntitySandMark extends AbstractOwnableMonster<EntitySandWitch>
{
    int timer = 10;
    
    public EntitySandMark(EntityType<? extends Monster> type, Level level) 
    {
        super(type, level);
        this.noCulling = true;
    ...
    public void aiStep() 
    {
        ...
        if(this.tickCount % 20 == 0) 
        {
            ...
            {
                List<Entity> targetList = this.level.getEntities(this, this.getBoundingBox().expandTowards(20.0, 20.0, 20.0));
            	if(this.getOwner() != null)
            	{
            		this.moveTo(this.getOwner().position().add(0, 4.0, 0));
            	}
                for(Entity entity : targetList) 
                {
                    if(entity instanceof Player)
                    {
                        ((Player)entity).addEffect(new MobEffectInstance(EWEffects.PARANOID.get(), 4000, 1));
                    }
                }
            }
