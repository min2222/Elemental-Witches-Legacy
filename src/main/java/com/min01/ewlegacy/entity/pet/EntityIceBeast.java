...
public class EntityIceBeast extends TamableAnimal
{
    public int deathTicks;
    public int involveTimer = 0;
    
    public EntityIceBeast(EntityType<? extends TamableAnimal> type, Level level)
    {
        super(type, level);
        ...
        this.targetSelector.addGoal(1, new HurtByTargetGoal(this).setAlertOthers(EntityIceBeast.class));
        this.setTame(true);
    }
    ...
    public boolean doHurtTarget(Entity target)
    {
        ...
        if(form <= 24) 
        {
            ((LivingEntity)target).addEffect(new MobEffectInstance(EWEffects.FROST_BITE.get(), 300, 1));
    ...
    public void aiStep() 
    {
        if(this.getCustomName().getString().equals("Flower Butt") && !this.level.isClientSide)
        {
            this.spawnAtLocation(new ItemStack(Blocks.POPPY).getItem(), 1);
            this.spawnAtLocation(new ItemStack(Blocks.DANDELION).getItem(), 1);
        }
    ...
    public boolean wantsToAttack(LivingEntity target, LivingEntity owner) 
    {
        ...
        {
            if(target instanceof EntityIceBeast) 
            {
            	EntityIceBeast entitywolf = (EntityIceBeast) target;
    ...
    public InteractionResult mobInteract(Player p_27584_, InteractionHand p_27585_)
    {
        ...
        {
            if(p_27584_.isHolding(EWItems.ICE_SHARD.get()))
            {
