...
public class EntityWaterNymph extends TamableAnimal
{
    public int deathTicks;
    public int involveTimer = 0;
    
    public EntityWaterNymph(EntityType<? extends TamableAnimal> type, Level level)
    {
        super(type, level);
        ...
        this.targetSelector.addGoal(1, new HurtByTargetGoal(this).setAlertOthers(EntityWaterNymph.class));
        this.setTame(true);
    }
    ...
    public boolean wantsToAttack(LivingEntity target, LivingEntity owner) 
    {
        ...
        {
            if(target instanceof EntityWaterNymph) 
            {
            	EntityWaterNymph entitywolf = (EntityWaterNymph) target;
    ...
    public InteractionResult mobInteract(Player p_27584_, InteractionHand p_27585_)
    {
        ...
        {
            if(p_27584_.isHolding(EWItems.WATER_SHARD.get()))
            {
                ...
                {
                    EntityWaterNymph2 mob = new EntityWaterNymph2(EWEntities.WATER_NYMPH_2.get(), this.level);
                    mob.moveTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), 0.0F);
                    if(this.getOwner() != null)
                    {
                        mob.setOwnerUUID(this.getOwnerUUID());
                    ...
                    for(Entity entity1 : list) 
                    {
                        ...
                        {
                        	//TODO
                        	//((Player)this.getOwner()).awardStat(MWAWAchievementsRegistry.waterpet3, 1);
                        }
                    }
                    this.discard();
                }
