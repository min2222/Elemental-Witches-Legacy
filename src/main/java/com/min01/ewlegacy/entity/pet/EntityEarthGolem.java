...
public class EntityEarthGolem extends TamableAnimal
{
    public int deathTicks;
    public int involveTimer = 0;
    
    public EntityEarthGolem(EntityType<? extends TamableAnimal> type, Level level)
    {
        super(type, level);
        ...
        this.targetSelector.addGoal(1, new HurtByTargetGoal(this).setAlertOthers(EntityEarthGolem.class));
        this.setTame(true);
    }
    ...
    public boolean wantsToAttack(LivingEntity target, LivingEntity owner) 
    {
        ...
        {
            if(target instanceof EntityEarthGolem) 
            {
                EntityEarthGolem entitywolf = (EntityEarthGolem) target;
    ...
    public InteractionResult mobInteract(Player p_27584_, InteractionHand p_27585_)
    {
        ...
        if(!p_27584_.isShiftKeyDown()) 
        {
            if(p_27584_.isHolding(EWItems.EARTH_SHARD.get()))
            {
                ...
                {
                	EntityEarthGolem2 mob = new EntityEarthGolem2(EWEntities.EARTH_GOLEM_2.get(), this.level);
                    mob.moveTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), 0.0F);
                    if(this.getOwner() != null)
                    {
                        mob.setOwnerUUID(this.getOwnerUUID());
                    ...
                    for(Entity entity1 : list) 
                    {
                        ...
                        {
                        	//TODO
                        	//((Player)this.getOwner()).awardStat(MWAWAchievementsRegistry.earthpet3, 1);
                        }
                    }
                    this.discard();
                }
