...
public class EntityAirPhoenix extends TamableAnimal
{
    public int deathTicks;
    int involveTimer = 0;
    
    public EntityAirPhoenix(EntityType<? extends TamableAnimal> type, Level level)
    {
        super(type, level);
        ...
        this.targetSelector.addGoal(1, new HurtByTargetGoal(this).setAlertOthers(EntityAirPhoenix.class));
        this.setTame(true);
    }
    ...
    public boolean wantsToAttack(LivingEntity target, LivingEntity owner) 
    {
        ...
        {
            if(target instanceof EntityAirPhoenix) 
            {
                EntityAirPhoenix entitywolf = (EntityAirPhoenix) target;
    ...
    public InteractionResult mobInteract(Player p_27584_, InteractionHand p_27585_)
    {
        ...
        if(!p_27584_.isShiftKeyDown()) 
        {
            if(p_27584_.isHolding(EWItems.AIR_SHARD.get()))
            {
                ...
                {
                    EntityAirPhoenix2 mob = new EntityAirPhoenix2(EWEntities.AIR_PHOENIX_2.get(), this.level);
                    mob.moveTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), 0.0F);
                    if(this.getOwner() != null)
                    {
                        mob.setOwnerUUID(this.getOwnerUUID());
                    ...
                    for(Entity entity1 : list) 
                    {
                        ...
                        {
                        	//TODO
                        	//((Player)this.getOwner()).awardStat(MWAWAchievementsRegistry.airpet3, 1);
                        }
                    }
                    this.discard();
                }
