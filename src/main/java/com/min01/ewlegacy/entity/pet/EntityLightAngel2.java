...
public class EntityLightAngel2 extends TamableAnimal
{
    ...
    public EntityLightAngel2(EntityType<? extends TamableAnimal> type, Level level)
    {
        super(type, level);
        ...
        this.targetSelector.addGoal(1, new HurtByTargetGoal(this).setAlertOthers(EntityLightAngel2.class));
        this.setTame(true);
    }
    ...
    public void aiStep() 
    {
        ...
        {
        	ElementalWitchesLegacy.PROXY.spawnParticlesWithMotion("sittingSignal", this.level, this.getX(), this.getY() + 2.9, this.getZ(), 0.0, 0.0, 0.0);
        }
    ...
    public boolean wantsToAttack(LivingEntity target, LivingEntity owner) 
    {
        ...
        {
            if(target instanceof EntityLightAngel2) 
            {
            	EntityLightAngel2 entitywolf = (EntityLightAngel2) target;
    ...
    public InteractionResult mobInteract(Player p_27584_, InteractionHand p_27585_)
    {
        ...
        if(!p_27584_.isShiftKeyDown()) 
        {
            if(p_27584_.isHolding(EWItems.LIGHT_SHARD.get()))
            {
                ...
                {
                    EntityLightAngel3 mob = new EntityLightAngel3(EWEntities.LIGHT_ANGEL_3.get(), this.level);
                    mob.moveTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), 0.0F);
                    if(this.getOwner() != null)
                    {
                        mob.setOwnerUUID(this.getOwnerUUID());
                    ...
                    for(Entity entity1 : list) 
                    {
                        ...
                        {
                        	//TODO
                        	//((Player)this.getOwner()).awardStat(MWAWAchievementsRegistry.lightpet4, 1);
                        }
                    }
                    this.discard();
                }
    ...
    {
        return -4.0F - Math.abs((float)Math.sin(this.floatingRotation) * 5.5F);
    }
}
