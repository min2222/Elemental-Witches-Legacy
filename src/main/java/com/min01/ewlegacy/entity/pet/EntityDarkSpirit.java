...
public class EntityDarkSpirit extends TamableAnimal
{
    ...
    public EntityDarkSpirit(EntityType<? extends TamableAnimal> type, Level level)
    {
        super(type, level);
        ...
        this.targetSelector.addGoal(1, new HurtByTargetGoal(this).setAlertOthers(EntityDarkSpirit.class));
        this.setTame(true);
    }
    ...
    public boolean wantsToAttack(LivingEntity target, LivingEntity owner) 
    {
        ...
        {
            if(target instanceof EntityDarkSpirit) 
            {
                EntityDarkSpirit entitywolf = (EntityDarkSpirit) target;
    ...
    public InteractionResult mobInteract(Player p_27584_, InteractionHand p_27585_)
    {
        ...
        if(!p_27584_.isShiftKeyDown()) 
        {
            if(p_27584_.isHolding(EWItems.DARK_SHARD.get()))
            {
                ...
                {
                    EntityDarkSpirit2 mob = new EntityDarkSpirit2(EWEntities.DARK_SPIRIT_2.get(), this.level);
                    mob.moveTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), 0.0F);
                    if(this.getOwner() != null)
                    {
                        mob.setOwnerUUID(this.getOwnerUUID());
                    ...
                    for(Entity entity1 : list) 
                    {
                        ...
                        {
                        	//TODO
                        	//((Player)this.getOwner()).awardStat(MWAWAchievementsRegistry.darkpet3, 1);
                        }
                    }
                    this.discard();
                }
    ...
    {
        return 15.0F - Math.abs((float)Math.sin(this.floatingRotation) * 5.5F);
    }
}
